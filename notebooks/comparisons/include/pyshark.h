...
// - https://github.com/pybind/pybind11/blob/master/include/pybind11/eigen.h
...
template <typename T>
using PySharkVector = shark::blas::vector<T>;

template <typename T>
using PySharkVectorList = std::vector<PySharkVector<T>>;
	...
	namespace detail
	{
		...
		public:
			...
			bool load(py::handle src, bool convert)
			{
				...
				value = remora::dense_vector_adaptor<T>((T *)buffer.data(), size, stride);
		...
		public:
			...
			bool load(py::handle src, bool convert)
			{
				...
				value = remora::dense_matrix_adaptor<T>((T *)buffer.data(), size1, size2, leading_dimension);

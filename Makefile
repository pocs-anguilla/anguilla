...
test:
	#python -m unittest
	pytest tests --color=yes --cov=anguilla --cov-report=xml --cov-config=pyproject.toml 
...
test_debug:
	...
	LD_PRELOAD="libasan.so" ASAN_OPTIONS=check_initialization_order=1 ASAN_OPTIONS=detect_leaks=0 pytest tests --color=yes --cov=anguilla --cov-report=xml --cov-config=pyproject.toml

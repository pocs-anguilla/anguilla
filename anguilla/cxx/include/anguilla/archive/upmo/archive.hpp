...
#ifndef ANGUILLA_ARHIVE_UPMO_ARCHIVE_HPP_
#define ANGUILLA_ARHIVE_UPMO_ARCHIVE_HPP_
...
Evolutionary Computation IV (pp. 155–169). Springer International Publishing.
...
template <typename T>
struct Node : public boost::intrusive::avl_set_base_hook<
                  boost::intrusive::optimize_size<true>> {
    explicit Node(const py::array_t<T>& point, const py::array_t<T>& fitness,
                  T step_size = 1.0, T p_succ = 0.5)
        : individual(point, fitness, step_size, p_succ) {
        individual.containerPtr = this;
    }

    friend bool operator<(const Node& l, const Node& r) {
        return l.individual < r.individual;
    }
...
template <typename T>
using BaseArchive =
    boost::intrusive::avl_set<Node<T>,
                              boost::intrusive::compare<std::less<Node<T>>>>;

template <typename T> class Archive;

template <typename T> struct NodeDisposer {
    void operator()(Node<T>* instancePtr) { delete instancePtr; }
};
...
template <typename T> class Archive {
  ...
  public:
    explicit Archive(
        ...
        : m_parameters(parameters), m_reference(reference) {
    ...
    ~Archive() { m_archive.clear_and_dispose(m_disposer); }

    [[nodiscard]] auto insert(const py::array_t<T>& point,
                              const py::array_t<T>& fitness) {
        auto newNode = new Node<T>(point, fitness, m_parameters.initialStepSize,
                                   m_parameters.pTargetSucc);
    ...
    [[nodiscard]] Individual<T>* insertInternal(Node<T>* newNode) {
        ...
        assert(nodeRight != m_archive.end());
        auto hintNode = m_archive.erase_and_dispose(std::next(nodeLeft),
                                                    nodeRight, m_disposer);

        // Insert point
        const auto pContribution = (nodeRight->individual.coord(0) - pX) *
                                   (nodeLeft->individual.coord(1) - pY);
        ...
        if (updateLeft) {
            ...
            const auto lContrib =
                (pX - nodeLeft->individual.coord(0)) *
                (prev->individual.coord(1) - nodeLeft->individual.coord(1));
        ...
        if (updateRight) {
            const auto rContrib =
                (next->individual.coord(0) - nodeRight->individual.coord(0)) *
                (pY - nodeRight->individual.coord(1));
    ...
    [[nodiscard]] Statistics getStatistics() const { return m_statistics; }
    ...
    [[nodiscard]] constexpr auto leftSentinel() { return m_archive.begin(); }
    ...
    nearest(Individual<T>* individualPtr) {
        auto* nodePtr =
            reinterpret_cast<NodePointer>(individualPtr->containerPtr);
        NodeIterator node;
        node = nodePtr; // no suitable constructor is available, so we use
                        // assignment
    ...
    [[nodiscard]] Individual<T>* sampleInterior(T p) {
        ...
        while (!result.has_value() || current == m_archive.end()) {
            if (!isExtremeNode(
                    static_cast<Node<T>*>(current->individual.containerPtr))) {
                auto contrib = std::pow(current->individual.contribution,
                                        m_parameters.alpha);
    ...
    [[nodiscard]] T _updateAccContributions(NodePointer& ptr) {
        ...
        if (!isExtremeNode(ptr)) {
            ...
            self->individual.accContribution =
                std::pow(self->individual.contribution, m_parameters.alpha);
        } else {
    ...
    [[nodiscard]] auto begin() {
        return ArchiveIterator(
            std::forward<NodeIterator>(std::next(m_archive.begin())));
    }

    [[nodiscard]] auto end() {
        return ArchiveIterator(
            std::forward<NodeIterator>(std::prev(m_archive.end())));
    }
...
template <typename T> struct Archive<T>::ArchiveIterator {
    ...
    explicit ArchiveIterator(NodeIterator&& nodeIterator)
        : m_iterator(nodeIterator) {}
    ...
    friend bool operator==(const ArchiveIterator& l, const ArchiveIterator& r) {
...
#endif // ANGUILLA_ARHIVE_UPMO_ARCHIVE_HPP_

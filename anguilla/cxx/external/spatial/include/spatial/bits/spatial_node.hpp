...
/**
     ...
     */
template <typename Link>
inline bool header(const Node<Link>* x) { return (x->left == x); }
...
increment(const Node<Link>* x) { return increment(const_cast<Node<Link>*>(x)); }
///@}
...
template <typename Link>
inline const Node<Link>*
decrement(const Node<Link>* x) { return decrement(const_cast<Node<Link>*>(x)); }
///@}
...
invariant_category(const Node<Link>*) { return typename Link::invariant_category(); }
...
const_key(const Node<Relaxed_kdtree_link<Value, Value> >* node) {
    return static_cast<const Relaxed_kdtree_link<Value, Value>*>(node)->value;
}
...
const_key(const Node<Relaxed_kdtree_link<Key, Value> >* node) {
    return static_cast<const Relaxed_kdtree_link<Key, Value>*>(node)->value.first;
}
...
class Const_node_iterator {
   ...
   public:
    ...
    //! Build and assign an interator to a link pointer.
    explicit Const_node_iterator(node_ptr x) : node(x) {}
...
struct Preorder_node_iterator {
   ...
   public:
    ...
    //! Build and assign an interator to a link pointer.
    explicit Preorder_node_iterator(node_ptr x) : node(x) {}
...
template <typename Link>
inline Node<Link>* increment(Node<Link>* x) {
    SPATIAL_ASSERT_CHECK(!header(x));
    if (x->right != 0) {
        x = x->right;
        while (x->left != 0) {
...
template <typename Link>
inline Node<Link>* decrement(Node<Link>* x) {
    SPATIAL_ASSERT_CHECK((!header(x) || x->parent != 0));
    if (header(x)) {
        x = x->right;
    }  // At header, 'right' points to the right-most node
    else if (x->left != 0) {
        Node<Link>* y = x->left;
        while (y->right != 0) {
            y = y->right;
...
template <typename Link>
inline void swap_node_aux(Node<Link>* a, Node<Link>* b) {

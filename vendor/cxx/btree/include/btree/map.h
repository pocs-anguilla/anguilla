...
class btree_map_container : public btree_unique_container<Tree> {
  ...
  public:
    ...
    std::pair<iterator, bool> try_emplace(const key_type& key, Args&&... args) {
        return this->__tree.emplace_unique_key_args(
    ...
    iterator try_emplace(const_iterator hint, const key_type& key,
                         Args&&... args) {
        return this->__tree.emplace_hint_unique_key_args(
...
bool operator==(const btree::map<K, V, C, A, N>& lhs,
                const btree::map<K, V, C, A, N>& rhs) {
...
template <typename K, typename V, typename C, typename A, int N>
bool operator==(const btree::multimap<K, V, C, A, N>& lhs,
                const btree::multimap<K, V, C, A, N>& rhs) {
    return lhs.size() == rhs.size() &&
           std::equal(lhs.begin(), lhs.end(), rhs.begin());
}
...
template <typename K, typename V, typename C, typename A, int N>
inline void swap(btree::multimap<K, V, C, A, N>& x,
                 btree::multimap<K, V, C, A, N>& y) {
    x.swap(y);
}
